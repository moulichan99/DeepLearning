...
Non-Max suppression
	In the figure above, we plotted only boxes for which the model had assigned a high probability, 
	but this is still too many boxes. You'd like to reduce the algorithm's output to a much smaller number of detected objects.
...
box_class_probs: tensor of shape  (19×19,5,80) containing the "class probabilities"  (c1,c2,...c80)(c1,c2,...c80)  for each of the 80 classes for each of the 5 boxes per cell.


Exercise: Implement yolo_filter_boxes().
Compute box scores by doing the elementwise product as described in Figure 4 ( p×cp×c ).
...
For this exercise, we define a box using its two corners: upper left  (x1,y1)(x1,y1)  and lower right  (x2,y2)(x2,y2) , instead of using the midpoint, height and width. (This makes it a bit easier to calculate the intersection).
To calculate the area of a rectangle, multiply its height  (y2−y1)(y2−y1)  by its width  (x2−x1)(x2−x1) . (Since  (x1,y1)(x1,y1)  is the top left and  x2,y2x2,y2  are the bottom right, these differences should be non-negative.
To find the intersection of the two boxes  (xi1,yi1,xi2,yi2)(xi1,yi1,xi2,yi2) :
Feel free to draw some examples on paper to clarify this conceptually.
The top left corner of the intersection  (xi1,yi1)(xi1,yi1)  is found by comparing the top left corners  (x1,y1)(x1,y1)  of the two boxes and finding a vertex that has an x-coordinate that is closer to the right, and y-coordinate that is closer to the bottom.
The bottom right corner of the intersection  (xi2,yi2)(xi2,yi2)  is found by comparing the bottom right corners  (x2,y2)(x2,y2)  of the two boxes and finding a vertex whose x-coordinate is closer to the left, and the y-coordinate that is closer to the top.
The two boxes may have no intersection. You can detect this if the intersection coordinates you calculate end up being the top right and/or bottom left corners of an intersection box. Another way to think of this is if you calculate the height  (y2−y1)(y2−y1)  or width  (x2−x1)(x2−x1)  and find that at least one of these lengths is negative, then there is no intersection (intersection area is zero).
...
Exercise: Implement yolo_eval() which takes the output of the YOLO encoding 
and filters the boxes using score threshold and NMS. There's just one last implementational detail you have to know. 
...
If you are testing this data on a different size image--for example, the car detection dataset had 720x1280 images--this step rescales the boxes so that they can be plotted on top of the original 720x1280 image.
...
def yolo_eval(yolo_outputs, image_shape = (720., 1280.), max_boxes=10, score_threshold=.6, iou_threshold=.5):
    ...
    3.3 - Convert output of the model to usable bounding box tensors
		...
		If you are curious about how yolo_head is implemented, you can find the function definition in the file 'keras_yolo.py'. 
		The file is located in your workspace in this path 'yad2k/models/keras_yolo.py
